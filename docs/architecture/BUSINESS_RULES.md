...
**Note on abstraction level (see ADR-004, `DECISION_LOG.md`):** The state
machine below is the canonical **domain-level** patient lifecycle — the
business-meaningful stages a queue entry passes through. It is not the
Queue Engine's internal operational state machine. `QUEUE_RULES.md`
§5.2–5.3 defines a finer-grained, mechanism-level state machine that
realizes this lifecycle inside the Queue Engine; the two are related by
refinement, not by conflict. The authoritative mapping between the two is
recorded in ADR-004 and must not be duplicated here.
...
### 10.1 Relationship to QUEUE_RULES.md (Non-Conflict Clarification)

This document is the domain authority (§1, §3) for *what* constitutes
correct business behavior, including which transitions are permitted.
`QUEUE_RULES.md` is the domain authority for *how* the Queue Engine
mechanically realizes each stage (state naming, retry/notification
mechanics, ordering). Where `QUEUE_RULES.md` names an internal state or
transition with no equivalent here (e.g. `REQUESTED`, `EXPIRED`, the
`NOTIFIED`/`CONFIRMED` split of Calling), that is expected refinement, not
a discrepancy requiring amendment of this document. Where a transition
permitted in `QUEUE_RULES.md` would contradict a constraint stated in this
document (for example, this document's prohibition in §4.9 and §10 on
cancelling a queue entry once Consultation has begun), this document's
constraint governs, and `QUEUE_RULES.md` is corrected to conform, per
ADR-004.
...
- No-show pattern reporting (e.g., flagging patients with repeated
  ...
  above.
- Formal handling of an interrupted or abandoned consultation (service
  began but did not conclude normally) — no business rule currently
  defines this case. Per ADR-004, the Queue Engine no longer models an
  `IN_SERVICE → CANCELLED` transition for this scenario (it contradicted
  §4.9/§10), so an interrupted consultation currently has no defined
  resolution and must be addressed by a future business rule before the
  Queue Engine can support it.
