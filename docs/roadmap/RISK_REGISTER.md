...
| ~~R-004~~ | ~~Top-level `queue/` directory/package shadows Python's standard library `queue` module, risking import shadowing bugs (affects asyncio, concurrent.futures, and any library doing `import queue`)~~ | ~~2~~ | ~~Medium~~ | ~~Medium-High~~ | ~~Resolved by ADR-006: renamed to `queue_engine/`~~ | **RESOLVED** — see ADR-006 |

## R-004 Resolution Details

- **ADR:** ADR-006 (`docs/adr/ADR-006-r004-resolution.md`)
- **Action:** Renamed `queue/` → `queue_engine/`
- **Date:** 2026-07-20
